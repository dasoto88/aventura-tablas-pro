...
function App() {
  ...
  useEffect(() => {
    if (modoDemo && demoInicio) {
      timerRef.current = setInterval(() => {
        ...
        if (elapsed >= 180) { clearInterval(timerRef.current); cerrarSesion(); }
      }, 1000);
    }
  ...
  return (
    <div className="app-root">
      <Toaster position="top-center" toastOptions={{ style: { background: "rgba(10,14,39,0.95)", color: "#fff", border: "1px solid rgba(0,245,255,0.4)", borderRadius: "12px", fontFamily: "'Rajdhani', sans-serif", fontSize: "16px" }, success: { iconTheme: { primary: "#00f5ff", secondary: "#0a0e27" } }, error: { iconTheme: { primary: "#ff006e", secondary: "#0a0e27" } } }} />
      {renderPagina()}
    </div>
  );
}
